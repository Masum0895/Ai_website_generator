    ...
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center gap-2 sm:min-w-90 text-nowrap'>
            ...
            <div>
                {isMenuOpen ? <MessageSquareIcon onClick={()=> setIsMenuOpen(true)} className='size-6 cursor-pointer'/>: <XIcon onClick={()=>setIsMenuOpen(true)} className='size-6 cursor-pointer'/>}
            </div>
        </div>
        {/* Midde */}
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
            ...
            <Link target='_blank' to={`/preview/${projectId}`} className='flex items-center px-4 py-1 gap-2 rounded sm:rounded-sm transition-colors border border-gray-700 hover:border-gray-500'>
